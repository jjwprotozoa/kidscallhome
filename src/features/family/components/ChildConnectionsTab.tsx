...
// Tab component for managing child-to-child connection requests and approvals - memoized to prevent re-renders on tab switch
...
// Type for adult profile query result (table not in generated types)
interface AdultProfileResult {
  id: string;
  family_id: string;
}

// Type for child profile query result (table not in generated types)
    ...
    const fetchConnections = useCallback(async () => {
      try {
        ...
        const { data: adultProfile } = (await (supabase as any)
          ...
          .eq("user_id", user.id)
          .eq("role", "parent")
          .single()) as { data: AdultProfileResult | null };

        if (!adultProfile) return;

        // Fetch connections (table not in generated types)
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        ...
        // Fetch child profiles for all needed children (table not in generated types)
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const { data: childProfiles } = (await (supabase as any)
          .from("child_profiles")
          .select("id, name")
          .in("id", Array.from(childIds))) as {
          data: ChildProfileResult[] | null;
        };
...
ChildConnectionsTab.displayName = "ChildConnectionsTab";

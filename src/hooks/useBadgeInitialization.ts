      ...
      try {
        ...
        const timeoutPromise = new Promise<never>((_, reject) => {
          timeoutId = setTimeout(() => {
            reject(new Error('Badge initialization timeout'));
          }, 10000);
        });
        ...
        // Don't log timeout errors as errors - they're expected if network is slow
        if (error instanceof Error && error.message === 'Badge initialization timeout') {
          console.warn("⏱️ [BADGE INIT] Initialization timed out - badges will populate via realtime");
        } else {

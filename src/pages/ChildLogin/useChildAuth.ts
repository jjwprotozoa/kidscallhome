...
export const useChildAuth = (skipFamilyCode: boolean) => {
  ...
  const handleLoginWithCode = useCallback(
    async (fullCode: string) => {
      ...
      try {
        ...
        if (!parseResult.valid || !parseResult.parsed) {
          ...
          try {
            // New table not yet in types (will be added after migration)
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            void (supabase.from as any)("child_login_attempts").insert({
              ip_address: clientIp,
              success: false,
            });
          } catch {
    ...
    [navigate, toast, skipFamilyCode, trackDevice]
  );

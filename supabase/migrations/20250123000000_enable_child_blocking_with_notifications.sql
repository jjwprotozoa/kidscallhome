...
-- Note: Children should use the get_child_blocks() function instead of direct SELECT
-- This provides better security since children use anonymous auth without auth.uid()
-- The function validates the child_profile_id and only returns their own blocks
...
-- Note: Children should use the unblock_contact_for_child() function instead of direct DELETE
-- This provides better security since children use anonymous auth without auth.uid()
-- The function validates that the block belongs to the child before unblocking
...
DROP POLICY IF EXISTS "Parents can select their own notifications" ON public.parent_notifications;
...
BEGIN
  ...
  -- Only create if one doesn't already exist for this block
  FOR parent_row IN
    ...
    WHERE ap.role = 'parent'
      ...
      AND NOT EXISTS (
        ...
        WHERE pn.parent_id = ap.id
          AND pn.blocked_contact_id = NEW.id
      )
  LOOP
